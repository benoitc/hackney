...
init([Name, Options]) ->
    ...
    {ok, #state{name=Name, max_connections=MaxConn, timeout=Timeout}}.
...
update_usage(#state{name=PoolName, sockets=Sockets, clients=Clients}) ->
    metrics:update([hackney_pool, PoolName,in_use_count], dict:size(Clients) - 1),
    metrics:update([hackney_pool, PoolName, free_count], dict:size(Sockets) - 1).
