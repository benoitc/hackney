...
%% A full handshake reporting h2 caches it.
learn_h2_then_recall() ->
    ?assertEqual(http2,
        hackney_ssl:resolve_alpn({ok, <<"h2">>}, false, none, "learn.example", h2())),
...
resumed_recall() ->
    ?assertEqual(http2,
        hackney_ssl:resolve_alpn({ok, <<"h2">>}, false, none, "res.example", h2())),
    ?assertEqual(http2,
...
http1_learn_and_recall() ->
    ?assertEqual(http1,
        hackney_ssl:resolve_alpn({ok, <<"http/1.1">>}, false, none, "h1.example", h1())),
    ?assertEqual(http1,
...
%% a real http1 even with http2 carried, and it overwrites the stale http2 so a
%% later resumption cannot recall it.
full_handshake_refreshes_stale_http2() ->
    ?assertEqual(http2,
        hackney_ssl:resolve_alpn({ok, <<"h2">>}, false, none, "ref.example", h2())),
    ?assertEqual(http1,
...
key_preserves_order_and_protocol_set() ->
    ?assertEqual(http2,
        hackney_ssl:resolve_alpn({ok, <<"h2">>}, false, none, "ord.example", h2())),
