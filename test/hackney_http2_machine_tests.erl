...
init_client_test() ->
    {ok, Preface, State} = hackney_cow_http2_machine:init(client, ?TEST_OPTS),
...
init_server_test() ->
    {ok, Preface, State} = hackney_cow_http2_machine:init(server, ?TEST_OPTS),
    ?assert(is_binary(iolist_to_binary(Preface))),
    ?assert(is_tuple(State)).

%% Test client can initialize a stream
init_stream_client_test() ->
    {ok, _Preface, State0} = hackney_cow_http2_machine:init(client, ?TEST_OPTS),
    %% Client needs to acknowledge server settings first
    %% Let's just test we can call init_stream
    {ok, StreamID, State1} = hackney_cow_http2_machine:init_stream(<<"GET">>, State0),
    ?assertEqual(1, StreamID),
    %% Next stream should be 3
    {ok, StreamID2, _State2} = hackney_cow_http2_machine:init_stream(<<"POST">>, State1),
    ?assertEqual(3, StreamID2).

%% Test stream IDs increment correctly
stream_id_increment_test() ->
    {ok, _Preface, State0} = hackney_cow_http2_machine:init(client, ?TEST_OPTS),
    {ok, 1, State1} = hackney_cow_http2_machine:init_stream(<<"GET">>, State0),
    {ok, 3, State2} = hackney_cow_http2_machine:init_stream(<<"GET">>, State1),
    {ok, 5, State3} = hackney_cow_http2_machine:init_stream(<<"GET">>, State2),
    {ok, 7, _State4} = hackney_cow_http2_machine:init_stream(<<"GET">>, State3).
...
init_with_custom_settings_test() ->
    ...
    {ok, Preface, _State} = hackney_cow_http2_machine:init(client, Opts),
    PrefaceBin = iolist_to_binary(Preface),
...
get_local_setting_test() ->
    {ok, _Preface, State} = hackney_cow_http2_machine:init(client, ?TEST_OPTS),
    %% Default initial window size is 65535
    ?assertEqual(65535, hackney_cow_http2_machine:get_local_setting(initial_window_size, State)).

%% Test get remote settings (defaults)
get_remote_settings_test() ->
    {ok, _Preface, State} = hackney_cow_http2_machine:init(client, ?TEST_OPTS),
    RemoteSettings = hackney_cow_http2_machine:get_remote_settings(State),
...
prepare_headers_get_test() ->
    ...
    {ok, IsFin, HeaderBlock, _State2} = hackney_cow_http2_machine:prepare_headers(
...
prepare_headers_post_test() ->
    {ok, _Preface, State0} = hackney_cow_http2_machine:init(client, ?TEST_OPTS),
    {ok, StreamID, State1} = hackney_cow_http2_machine:init_stream(<<"POST">>, State0),
    PseudoHeaders = #{
        method => <<"POST">>,
        scheme => <<"https">>,
        authority => <<"example.com">>,
...
settings_frame_test() ->
    ...
    {ok, State1} = hackney_cow_http2_machine:frame(SettingsFrame, State0),
...
settings_ack_frame_test() ->
    ...
    {ok, State2} = hackney_cow_http2_machine:frame(settings_ack, State1),
...
ping_frame_test() ->
    ...
    {ok, State1} = hackney_cow_http2_machine:frame({settings, #{}}, State0),
    %% Now handle PING
    PingFrame = {ping, 12345},
    {ok, State2} = hackney_cow_http2_machine:frame(PingFrame, State1),
    ?assert(is_tuple(State2)).
...
ping_ack_frame_test() ->
    ...
    {ok, State2} = hackney_cow_http2_machine:frame(PingAckFrame, State1),
    ?assert(is_tuple(State2)).
...
goaway_frame_test() ->
    ...
    {ok, {goaway, 0, no_error, <<>>}, _State2} = hackney_cow_http2_machine:frame(GoawayFrame, State1).
...
update_window_connection_test() ->
    {ok, _Preface, State0} = hackney_cow_http2_machine:init(client, ?TEST_OPTS),
    State1 = hackney_cow_http2_machine:update_window(1000, State0),
    ?assert(is_tuple(State1)).

%% Test get connection local buffer size
get_connection_buffer_size_test() ->
    {ok, _Preface, State} = hackney_cow_http2_machine:init(client, ?TEST_OPTS),
    BufferSize = hackney_cow_http2_machine:get_connection_local_buffer_size(State),
...
get_stream_local_state_test() ->
    {ok, _Preface, State0} = hackney_cow_http2_machine:init(client, ?TEST_OPTS),
    {ok, StreamID, State1} = hackney_cow_http2_machine:init_stream(<<"GET">>, State0),
    {ok, LocalState, BufferState} = hackney_cow_http2_machine:get_stream_local_state(StreamID, State1),
...
get_stream_remote_state_test() ->
    {ok, _Preface, State0} = hackney_cow_http2_machine:init(client, ?TEST_OPTS),
    {ok, StreamID, State1} = hackney_cow_http2_machine:init_stream(<<"GET">>, State0),
    {ok, RemoteState} = hackney_cow_http2_machine:get_stream_remote_state(StreamID, State1),
    ?assertEqual(idle, RemoteState).

%% Test stream not found
get_stream_state_not_found_test() ->
    {ok, _Preface, State} = hackney_cow_http2_machine:init(client, ?TEST_OPTS),
    ?assertEqual({error, not_found}, hackney_cow_http2_machine:get_stream_local_state(999, State)),
    ?assertEqual({error, not_found}, hackney_cow_http2_machine:get_stream_remote_state(999, State)).
...
last_streamid_test() ->
    ...
    LastStreamID = hackney_cow_http2_machine:get_last_streamid(State0),
    ?assertEqual(0, LastStreamID),
    %% Set last streamid (returns current remote streamid)
    {RemoteStreamID, State1} = hackney_cow_http2_machine:set_last_streamid(State0),
...
is_lingering_stream_test() ->
    {ok, _Preface, State} = hackney_cow_http2_machine:init(client, ?TEST_OPTS),
    ?assertEqual(false, hackney_cow_http2_machine:is_lingering_stream(1, State)).
