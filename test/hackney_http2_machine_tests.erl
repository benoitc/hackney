...
init_client_test() ->
    {ok, Preface, State} = hackney_cow_http2_machine:init(client, #{}),
...
init_server_test() ->
    {ok, Preface, State} = hackney_cow_http2_machine:init(server, #{}),
    ?assert(is_binary(iolist_to_binary(Preface))),
    ?assert(is_tuple(State)).

%% Test client can initialize a stream
init_stream_client_test() ->
    {ok, _Preface, State0} = hackney_cow_http2_machine:init(client, #{}),
...
get_local_setting_test() ->
    {ok, _Preface, State} = hackney_cow_http2_machine:init(client, #{}),
